...
import com.rch.etawah.ActivityUtil.ProductDetail;
import com.rch.etawah.AdapterUtil.RestaurantMenuAdapter;
import com.rch.etawah.ConstantUtil.Constant;
import com.rch.etawah.InterfaceUtil.ConnectionCallback;
import com.rch.etawah.InterfaceUtil.ProductCallback;
...
public class MenuFragment extends Fragment implements ProductCallback, ConnectionCallback {
    ...
    public void onResume() {
        ...
        ((DashboardFragment) getParentFragment()).setListener(categoryPosition, (i, s) -> getActivity().runOnUiThread(() -> {
            final String value = String.valueOf(s).toLowerCase().trim();
            Utility.Logger("CharSequence", String.valueOf(s));

            Map<String, DataObject> items = new HashMap<>();
            for (DataObject object : mDataObject) {
                ...
                if (postName.contains(value)) {
                    items.put(postName, object);
                }
            }
            objectArrayList.clear();

            for (Map.Entry<String, DataObject> pair : items.entrySet()) {
                Utility.Logger("CharSequence", pair.getKey());
                objectArrayList.add(pair.getValue());
            }
            restaurantMenuAdapter.notifyDataSetChanged();
        }));
